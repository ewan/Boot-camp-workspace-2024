...
W = torch.nn.Linear(50, hid_size, dtype=dtype)
B = torch.nn.Linear(hid_size, hid_size, dtype=dtype)
M = torch.nn.Linear(hid_size, 1, dtype=dtype)

loss_fn = torch.nn.BCEWithLogitsLoss()
optimizer = torch.optim.Adam(list(W.parameters()) + list(W.parameters()) +
                             list(M.parameters()), lr=0.001)

def do_rnn(X):
    h = torch.zeros(hid_size, dtype=dtype)
    for i in range(X.shape[0]):
        h = torch.nn.functional.tanh(torch.Tensor.add(B(h), W(X[i,:])))
    z = M(h).squeeze()
    return z
